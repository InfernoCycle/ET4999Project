<?php 
  ...
  $result = $obj->any("SELECT user_id, is_in, first_name, last_name, table_id, email FROM users WHERE cancel_id <= 200000;");
...
<body>
  <?php if(isset($_SESSION['logged'])){?>
    <form id="loggedOut" action="./requests/logout.php" method="POST">
  ...
  <div id="cancel_modal2">
    <p id="accepted_msg2" class="confirm_question">Checked In</p>
    <p class="confirm_question">&#x2713;</p>
    <button type="button" class="ok_close_emp_modal supplement_ok_close">Close</button>
  </div>
    ...
    <div style="width:100%; text-align:center;">
        ...
        <fieldset id="status_fieldset">
          ...
          <datalist id="registrations">
            <?php while($count < count($res_array)){ 
              ...
              <option id=<?=$res_array[$count][0]?> value="<?=$res_array[$count][2]." ".$res_array[$count][3].",".$inner_res['reserved_date'].",".$inner_res['reserved_time'].",".get_table_by_id($res_array[$count][4]).",".$res_array[$count][5];?>" ><?=$res_array[$count][2]." ".$res_array[$count][3]." for ".$inner_res['reserved_date']." at ".$inner_res['reserved_time'].", ".get_table_by_id($res_array[$count][4]);?></option>
              <?php $count+=1;}?>
            <?php $obj->close();?>
          </datalist>
          <p class="space"></p>
      ...
      <form class="custom_form custom_form_v2" id="table_in_form3" action="status.php" method="POST">
        <fieldset id="status_fieldset">
          <legend style="padding:10px; font-weight: bolder; font-size:35px">Out of Order</legend>
          <label class="employee_forms_labels">Canellation Area </label>
          <br>
          <button id="cancel_out_btn" class="employee_submits" type="button">Broken</button>
        </fieldset>
      </form>
    </div>
    <footer></footer>
    ...
    <script>
      ...
      $(document).ready(function(){
        ...
        $("#accept_rsv_btn").on("click", function(e){
          $.post("./requests/accept.php", {option:user_id, xm99l:email}, function(data){
        ...
        $("#walk_in_btn").on("click", function(e){
          const form_second = document.getElementById("table_in_form2");
          const table_type = form_second.getElementsByTagName("select").namedItem("table_types");
          const fit = table_type.value.split(",")[0];
          
          $.post("./requests/walk_in_cust.php", {xf32m:true, xf32_is_in:fit}, function(data){
            console.log(data, 99);
            if("true" === data.toString()){
              /*const acc_msg_v2 = document.getElementById("accepted_msg2");
              acc_msg_v2.innerText = "Checked In";*/
              box_out_body_cancelModal();
            }else if(data.toString() === "Error"){
              /*const acc_msg_v2 = document.getElementById("accepted_msg2");
              acc_msg_v2.innerText = "An Error Occurred. Please Try Again Later or Notify IT Div";*/
            }
          });
        });
      });
